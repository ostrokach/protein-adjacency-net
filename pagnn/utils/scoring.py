...
def _get_blosum62():
    BLOSUM62 = np.zeros((len(AMINO_ACIDS), len(AMINO_ACIDS)), dtype=np.float32)
    ...
    for key, value in _blosum_62_dict.items():
        if key[0] not in AMINO_ACIDS or key[1] not in AMINO_ACIDS:
            continue
        idx1 = AMINO_ACIDS.index(key[0])
        idx2 = AMINO_ACIDS.index(key[1])
        BLOSUM62[idx1, idx2] = value
        BLOSUM62[idx2, idx1] = value
...
def score_blosum62(target: torch.FloatTensor, decoys: torch.FloatTensor) -> float:
    ...
    return ((blosum62 @ target) * decoys).sum(1).sum(1).max() / target.shape[1]
...
def score_edit(target: torch.FloatTensor, decoys: torch.FloatTensor) -> float:
    ...
    return ((target != 0) & (target == decoys)).sum(1).sum(1).max() / target.shape[1]
