...
def filter_mismatch_mutations(df_ref: pd.DataFrame) -> pd.DataFrame:
    ...
    logger.info(f"Removing {(~df['mutation_in_domain']).sum()} rows "
    ...
    logger.info(f"Removing {(~df['mutation_matches_sequence']).sum()} rows "
    ...
    logger.info(f"Removing {(~df['mutation_in_alignment']).sum()} rows "
    ...
    columns_to_drop = [
        'mutation_in_domain', 'mutation_matches_sequence', 'mutation_in_alignment',
