...
def generate_examples(args: Args, dataset: DataSetGAN, net_d, net_g) -> pd.DataFrame:
    ...
    target = torch.cat([datavar.seqs[0, :, :].data] * args.batch_size, 1)
    ...
    for _ in tqdm.tqdm(range(args.nseqs), total=args.nseqs, disable=not settings.SHOW_PROGRESSBAR):
        ...
        with torch.no_grad():
            ...
            assert fake_pred.shape[2] == (min_length * args.batch_size / 4 + 1)
        ...
        for i, start in enumerate(range(0, fake_seq_onehot.shape[2], datavar.seqs.shape[2])):
            ...
            score = score.sigmoid().mean()
            if best_score is None or score > best_score:
                best_score = score
                best_idx = i
                fake_seq_slice = fake_seq_onehot[:, :, start:stop]
                best_blosum62_score = float(
                    pagnn.utils.score_blosum62(datavar.seqs.data, fake_seq_slice))
                best_edit_score = float(pagnn.utils.score_edit(datavar.seqs.data, fake_seq_slice))
