...
def generate_examples(
    args: Args, dataset: DataSetGAN, datagen_ref: DataSetGAN, net_d, net_g, random_state
) -> pd.DataFrame:
    ...
    adjs_batch = (
        adjs_ref[: args.batch_size // 2] + [datavar.adjs] + adjs_ref[args.batch_size // 2 :]
    )
    ...
    for _ in tqdm.tqdm(range(args.nseqs), total=args.nseqs, disable=not settings.SHOW_PROGRESSBAR):
        ...
        df_columns["sequence"].append(
            "".join(pagnn.AMINO_ACIDS[int(i)] for i in np.argmax(fake_seq_slice.data.numpy(), 1)[0])
        )
