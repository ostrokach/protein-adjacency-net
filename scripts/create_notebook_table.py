...
def md_to_rst(md_text):
    ...
    md_text = " ".join(
        l.strip() for l in md_text if not l.startswith("#") and not l.startswith("---") if l.strip()
    )
...
def main(notebook_dir, output_file):
    ...
    with open(output_file, "w", newline="") as csv_file:
        ...
        for notebook_name, notebook_description in notebooks:
            notebook_url = "`{} <notebooks/{}.html>`_".format(
                notebook_name, op.splitext(notebook_name)[0]
            )
